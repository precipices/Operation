...
public class SelectNursePane extends BackPane {
	BackToolBar toolBar;
			...
			public void actionPerformed(ActionEvent arg0) {
				if (selectType.equals("按日期查询")) {
					...
					if (InitComponent.helper == null) {
						JOptionPane.showMessageDialog(SelectNursePane.this, "未连接服务器!");
					} else {
						Date begin = null;
						try {
							begin = Date.valueOf(beginDate.getText());
						} catch (Exception e) {
							JOptionPane.showMessageDialog(SelectNursePane.this, "请选择日期!");
							return;
						}
						if (begin.getTime() > new java.util.Date().getTime() - 24 * 3600 * 1000) {
							...
							SelectNursePane.this.setRowData(workers);
						} else {
							JOptionPane.showMessageDialog(SelectNursePane.this, "不可查询过去的排班!");
						}
					}
					...
					if (InitComponent.helper == null) {
						...
						SelectNursePane.this.setRowData(worker);
					}
	...
	public static void main(String[] args) {
		SelectNursePane p = new SelectNursePane();
