			...
			public void actionPerformed(ActionEvent arg0) {
				if (selectType.equals("按日期查询")) {
					...
					if (InitComponent.helper == null) {
						JOptionPane.showMessageDialog(SelectMyOperationPane.this, "未连接服务器!");
					} else {
						Date begin = null;
						Date end=null;
						try {
							...
							JOptionPane.showMessageDialog(SelectMyOperationPane.this, "请选择日期!");
							return;
						}
						if (begin.getTime() > new java.util.Date().getTime() - 24 * 3600 * 1000) {
							System.out.println("begin.getTime()>now.getTime()-24*3600*1000");
						} else {
							JOptionPane.showMessageDialog(SelectMyOperationPane.this, "不可查询过去的排班!");
							return;
						}
						if(begin.getTime()>end.getTime()) {
							JOptionPane.showMessageDialog(SelectMyOperationPane.this, "开始时间 不可超过结束时间!");
							return;
						}else {
