...
public class SelectDoctorPane extends BackPane {
	BackToolBar toolBar;
			...
			public void actionPerformed(ActionEvent arg0) {
				if (selectType.equals("按日期查询")) {
					...
					if (InitComponent.helper == null) {
						JOptionPane.showMessageDialog(SelectDoctorPane.this, "未连接服务器!");
					} else {
						Date begin = null;
						try {
							begin = Date.valueOf(beginDate.getText());
						} catch (Exception e) {
							JOptionPane.showMessageDialog(SelectDoctorPane.this, "请选择日期!");
							return;
						}
						if (begin.getTime() > new java.util.Date().getTime() - 24 * 3600 * 1000) {
							System.out.println("begin.getTime()>now.getTime()-24*3600*1000");
							Vector<Worker> workers = InitComponent.helper.selectDoctorsByDate(begin);
							SelectDoctorPane.this.setRowData(workers);
						} else {
							JOptionPane.showMessageDialog(SelectDoctorPane.this, "不可查询过去的排班!");
						}
					}
					...
					if (InitComponent.helper == null) {
						...
						SelectDoctorPane.this.setRowData(worker);
					}
	...
	public static void main(String[] args) {
		SelectDoctorPane p = new SelectDoctorPane();
